...
void Renderer::Draw()
{
	...
	SharedProjectionAndViewing::setProjectionMatrix(mProjectionMat);
	SharedProjectionAndViewing::setViewMatrix(mViewMat);
...
bool Renderer::LoadShaders()
{
	...
	// Set identity matix as the viewing matrix
	mViewMat = glm::lookAt( vec3( 0, 0, 12 ), vec3( 0, 0, -1 ), vec3( 0, 1, 0 ) );
...
void Renderer::Resize( )
{
	...
	mSpriteShader->SetMatrixUniform( "uViewProj", mProjectionMat );

	// Set the active shader
	mMeshShader->SetActive( );
	mProjectionMat = glm::perspective( glm::radians( 45.0f ), static_cast<float>( mScreenWidth ) / mScreenHeight, 1.0f, 200.0f );

	// Set the view-projection and view matrice
	SharedProjectionAndViewing::setProjectionMatrix( mProjectionMat );
	SharedProjectionAndViewing::setViewMatrix(mViewMat);
}
